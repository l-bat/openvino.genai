...
TEST_F(TaylorSeerStateTest, ResetClearsState) {
    TaylorSeerState state;
    ASSERT_FALSE(state.get_last_update_step().has_value());

    auto tensor = CreateTestTensor({1.0f, 2.0f, 3.0f});
    state.update(5, tensor);
    ASSERT_TRUE(state.get_last_update_step().has_value());
    EXPECT_EQ(state.get_last_update_step().value(), 5);
