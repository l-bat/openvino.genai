...
public:
    ...
    void update_scores(const ov::Tensor& attention_scores) {
        ...
        ).set_shape(ov::Shape{attn_shape[1] - m_eviction_config.start_size});
        ...
        if (!m_scores) {
            ...
            if (m_eviction_config.aggregation_mode == AggregationMode::NORM_SUM) {
                ...
                m_cache_counter = ov::Tensor(ov::element::Type::u64, ov::Shape{scores_size}, counter.data());
            }
        } else {
            auto m_scores_data = m_scores.data<float>();
            auto hh_scores_data = hh_score.data<float>();
                for (size_t i = 0; i < m_scores.get_size(); ++i) {
                    hh_score[i] += m_scores[i];
                }
            }
            m_scores = hh_score;

            num_new_tokens = hh_score.get_size() - m_scores.get_size();
            if (m_eviction_config.aggregation_mode == AggregationMode::NORM_SUM) {
                std::vector<std::size_t> new_tokens_counter(hh_score.get_size());
                for (size_t i = 0; i < m_cache_counter.get_size(); ++i) {
                    new_tokens_counter[i] = tensor_data[i];
                }
                ...
                m_cache_counter = ov::Tensor(ov::element::Type::u64, ov::Shape{hh_score.get_size()}, new_tokens_counter.data());
            }
        }
    }
    ...
    void remove_evicted_scores(const std::vector<std::size_t>& evict_block_indices) {
        ...
        for (size_t i = 0, size_t it = 0; i < m_scores.get_size();) {
        ...
        m_cache_counter = ov::Tensor(m_cache_counter_data.get_element_type(), ov::Shape{new_size}, counter.data());
    }
